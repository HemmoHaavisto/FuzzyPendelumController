...
# ╔═╡ 982f15c0-2df7-11f0-1a8a-97b4703dea14
using Plots, ControlSystems, LinearAlgebra, DifferentialEquations


# ╔═╡ 9af3f065-91cf-4e91-9aa9-dd31f7d4748f
...
begin
	...
	den = (mc + mp) * (1/3 * mp * l^2 + mp * l^2) - (mp * l)^2
	...
	function pid_dynamics!(du, u, p, t)
	    θ, θ̇, x, ẋ, eint = u
	    error = 0 - θ
	    derror = -θ̇
	
	    F = Kp * error + Ki * eint + Kd * derror
	
	    denom = (4/3)*(mc + mp) - mp * cos(θ)^2
	    ẍ = ((4/3)*F - (4/3)*mp*l*θ̇^2*sin(θ) - mp*g*sin(θ)*cos(θ)) / denom
	    ...
	    du[3] = ẋ
	    du[4] = ẍ
	    du[5] = error  #virheen integraali
	end
	
	u0 = [0.25, 0.0, 0.0, 0.0, 0.0]  # [θ, θ̇, x, ẋ, ∫e]
	tspan = (0.0, 30.0)
	
	prob = ODEProblem(pid_dynamics!, u0, tspan)
	sol = solve(prob, Tsit5()) # chatGPT ehdotti :D
end

# ╔═╡ d1a3c30c-7402-41ee-9b65-aadb1916e9c0
begin
	# --- Aikaleima- ja tilasarjat ---
	t = sol.t
	θs = sol[1, :]
	xs = sol[3, :]
	Fs = Kp * (0 .- θs) .+ Ki * cumsum(0 .- θs) * (t[2] - t[1]) .- Kd * sol[2, :]  # arvioitu ohjausvoima
	
	# --- Piirto ---
	p1 = plot(t, θs, label = "θ(t)", xlabel = "Aika (s)", ylabel = "Kulma (rad)",
	          title = "Heilurin kulma", lw = 2)
	
	p2 = plot(t, xs, label = "x(t)", xlabel = "Aika (s)", ylabel = "Vaunun sijainti (m)",
	          title = "Vaunun paikka", lw = 2)
	
	p3 = plot(t, Fs, label = "F(t)", xlabel = "Aika (s)", ylabel = "Ohjausvoima (N)",
	          title = "Ohjausvoima", lw = 2)
...
manifest_format = "2.0"
project_hash = "d27ac72d4932af9b0d6d64981449f82e43854c1e"
...
git-tree-sha1 = "61c891b8ea789d9f958271071661237ec4a8f835"
...
git-tree-sha1 = "1dd6260b553a61426491f78a46b61158234f2e60"
...
git-tree-sha1 = "36a36a44e38017176360bf39b5036588561c7bde"
