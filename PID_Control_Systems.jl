...
begin
	...
	sys_theta = ss(A, B, C, D)
	sys_all = ss(A, B, Matrix{Float64}(I, 4, 4), zeros(4, 1))
end

# ╔═╡ 8150a861-c535-4511-925f-82f2d2a5cc75
begin
	Kp = 50.0
	Ki = 1.0
	Kd = 10.0
...
# ╔═╡ 4b4ee064-5650-4144-8cd9-0f4fc19c6254
begin
# --- Simulaatio: silmukka ---
	for i in 1:n
	    e = θ_target - x[1]
	    de = -x[2]
	    integral = sum(t[1:i] .* (θ_target .- θs[1:i])) * dt 
...
# ╔═╡ 97e206fb-460d-419d-b311-98506c4b9bb5
begin
	# --- Plottaus ---
	p1 = plot(t, θs, label="θ(t)", xlabel="Aika (s)", ylabel="Kulma (rad)", title="Heilurin kulma", lw=2)
	p2 = plot(t, xs, label="x(t)", xlabel="Aika (s)", ylabel="Paikka (m)", title="Vaunun sijainti", lw=2)
	p3 = plot(t, Fs, label="F(t)", xlabel="Aika (s)", ylabel="Voima (N)", title="Ohjausvoima", lw=2)
