...
# ╔═╡ 4b075094-9272-4010-90f8-f18cc0ef9b27
using DifferentialEquations, Plots

# ╔═╡ 833dbd90-2dfe-11f0-011e-19ebf00274a7
begin
	# --- Parametrit ---
	mp = 0.1
	mc = 1.0
	...
	function pid_dynamics!(du, u, p, t)
	    ...
	    # PID-ohjaus
	    F_unclamped = Kp * error + Ki * eint + Kd * derror
	    F = clamp(F_unclamped, -F_max, F_max)
	
	    # Anti-windup: päivitä integraalivirhe vain jos ei saturaatiossa
	    du[5] = (F == F_unclamped) ? error : 0.0
	...
	# Laske ohjausvoima jälkikäteen
	Fs = [clamp(Kp * (-θ) + Ki * e + Kd * (-θ̇), -F_max, F_max) for (θ, θ̇, e) in zip(θs, θ̇s, es)]
	
	# --- Piirrä tulokset ---
	p1 = plot(ts, θs, label = "θ(t)", xlabel = "Aika (s)", ylabel = "Kulma (rad)",
	          title = "Heilurin kulma", lw = 2)
	p2 = plot(ts, xs, label = "x(t)", xlabel = "Aika (s)", ylabel = "Vaunun sijainti (m)",
	          title = "Vaunun paikka", lw = 2)
	p3 = plot(ts, Fs, label = "F(t)", xlabel = "Aika (s)", ylabel = "Ohjausvoima (N)",
